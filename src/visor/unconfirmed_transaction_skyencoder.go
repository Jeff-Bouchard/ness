...

	{
		...
		if length != 0 {
			...
			for z2 := range obj.Transaction.Out {
				...
				{
					// obj.Transaction.Out[z2].Delegate.Version
					i, err := d.Uint8()
					if err != nil {
						return 0, err
					}
					obj.Transaction.Out[z2].Delegate.Version = i
				}

				{
					// obj.Transaction.Out[z2].Delegate.Key
					if len(d.Buffer) < len(obj.Transaction.Out[z2].Delegate.Key) {
						return 0, encoder.ErrBufferUnderflow
					}
					copy(obj.Transaction.Out[z2].Delegate.Key[:], d.Buffer[:len(obj.Transaction.Out[z2].Delegate.Key)])
					d.Buffer = d.Buffer[len(obj.Transaction.Out[z2].Delegate.Key):]
				}

				{
					// obj.Transaction.Out[z2].MaxHours
					i, err := d.Uint64()
					if err != nil {
						return 0, err
					}
					obj.Transaction.Out[z2].MaxHours = i
				}

				{
					// obj.Transaction.Out[z2].Expiry
					i, err := d.Uint64()
					if err != nil {
						return 0, err
					}
					obj.Transaction.Out[z2].Expiry = i
				}

				{
					// obj.Transaction.Out[z2].MinInterval
					i, err := d.Uint64()
					if err != nil {
						return 0, err
					}
					obj.Transaction.Out[z2].MinInterval = i
				}

				{
					// obj.Transaction.Out[z2].DelegateLast
					i, err := d.Uint64()
					if err != nil {
						return 0, err
					}
					obj.Transaction.Out[z2].DelegateLast = i
				}
